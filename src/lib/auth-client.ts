...
export type OrderTracking={status:string;label:string;trackingNumber:string|null;updatedAt:string|null;history:Array<{status:string;label:string;occurredAt:string}>};
export type OrderDetail=OrderSummary & {tracking?:OrderTracking;delivery?:{label:string;city:string;address:string};statusHistory?:Array<{kind:string;status:string;occurred_at:string}>;shipment:null|{carrier:string;trackingNumber:string};subtotal_minor:number;delivery_minor:number;canCancel:boolean;items:Array<{sku:string;name_snapshot:string;quantity:number;unit_minor:number;line_minor:number}>};
export const trackingLabels:Record<string,string>={pending_payment:'Ожидает оплаты',paid:'Оплачен',assembling:'Собираем заказ',handed_to_cdek:'Передан в СДЭК',in_transit:'В пути',out_for_delivery:'У курьера',ready_for_pickup:'Готов к выдаче',delivered:'Получен',cancelled:'Отменён',returning:'Возврат',refunded:'Деньги возвращены',review:'Уточняем статус'};
...
export function parseOrderDetail(raw:unknown):OrderDetail {
 ...
 if(row.delivery!==undefined){const d=object(row.delivery);if(!['label','city','address'].every(k=>typeof d[k]==='string'&&d[k].length<=5000))throw new AuthClientError('INVALID_RESPONSE');delivery={label:String(d.label),city:String(d.city),address:String(d.address)};}
