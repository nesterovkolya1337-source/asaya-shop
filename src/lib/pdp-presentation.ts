...
export function richSections(pdp?:PdpContent):PdpSection[]{
 ...
 return pdp.sections.filter(s=>s.visible!==false).filter(s=>s.kind==='faq'?s.items.some(i=>i.title.trim()&&i.body.trim()):!!(s.body.trim()||s.additionalBody.trim()||s.media.length||s.items.some(i=>i.title.trim()||i.body.trim()||i.media)));
}
