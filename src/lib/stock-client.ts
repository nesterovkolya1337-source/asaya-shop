...
export type StockReport={configured:boolean;source:null|{mappingWarnings?:string[];externalWarehouseId:string;environment:'test'|'production';syncedAt:string|null;sourceUpdatedAt:string|null;expiresAt:string|null;nextAttemptAt:string|null;syncStatus:'fresh'|'stale'|'error'|'never_synced';lastError:string|null};items:StockItem[]};
