...
export function ProductRichContent({content}:{content?:PdpContent}){
 ...
 if(s.kind==='howTo')return <section key={s.kind} className={styles.howTo}>{copy}<div className={styles.steps}>{items.map((i,n)=><article key={n}>{i.media&&<Visual media={[i.media]} ratio={435/420}/>}<div>{i.title.trim()&&<h3>{i.title}</h3>}{i.body.trim()&&<p>{i.body}</p>}</div></article>)}</div>{s.media.length>0&&<div className={styles.steps}>{s.media.map((m,n)=><Media key={n} media={m}/>)}</div>}</section>;
