...
export function AdminStocks({csrf,onExpired}:{csrf:string;onExpired:()=>void}){
 ...
 {Boolean(source.mappingWarnings?.length)&&<p role="alert" className={styles.notice}>Нет опубликованных карточек ASAYA для артикулов СДЭК: {source.mappingWarnings?.join(', ')}. Эти артикулы пропущены. Остатки остальных товаров обновлены.</p>}
