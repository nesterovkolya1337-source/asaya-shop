...
export function AdminBanner({csrf,onExpired}:Props){
 ...
 return <section aria-label="Предупреждающий баннер"><h2>Предупреждающий баннер</h2><p>Показывается только по решению администратора.</p>{notice&&<p role="status">{notice}</p>}{v&&<form onSubmit={e=>{e.preventDefault();if(busy)return;setBusy(true);void request('banner','PUT',v,csrf).then(r=>{setV({...v,revision:(r as {revision:number}).revision});setNotice('Баннер сохранён.');}).catch(error).finally(()=>setBusy(false));}}><fieldset disabled={busy}><label><input type="checkbox" checked={v.enabled} onChange={e=>setV({...v,enabled:e.target.checked})}/>Показывать баннер</label><label>Текст сообщения<textarea required={v.enabled} maxLength={2000} value={v.message} onChange={e=>setV({...v,message:e.target.value})}/></label><label>Текст кнопки<input maxLength={100} value={v.buttonText} onChange={e=>setV({...v,buttonText:e.target.value})}/></label><label>Ссылка кнопки<input maxLength={1000} value={v.buttonUrl} onChange={e=>setV({...v,buttonUrl:e.target.value})}/></label><button>Сохранить баннер</button></fieldset></form>}</section>;
}
