...
export function ServerCartView(){
 ...
 {rows.length?rows.map(({id,quantity,product:p,purchasableQuantity,state})=><article className={styles.row+' '+(purchasableQuantity===0?styles.unavailable:'')} key={id} data-cart-id={id}>
 <div><h2>{p?.name??'Товар временно недоступен'}</h2><p>{p?rubles(Math.round(p.price*100)*purchasableQuantity):'Цена недоступна'}</p>
