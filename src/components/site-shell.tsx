      ...
      {searchOpen && <div className={styles.searchBackdrop} onClick={() => setSearchOpen(false)}>
        <section aria-label="Поиск по сайту" aria-modal="true" className={styles.searchPanel} onClick={(event) => event.stopPropagation()} role="dialog">
          ...
          <div className={styles.searchResults}>
            {searchResults.map((product) => <Link href={`/product/${product.id}`} key={product.id} onClick={() => setSearchOpen(false)}><CroppedImage alt="" height={70} src={product.image} width={52} /><span><strong>{product.name}</strong><small>Товар · {product.volume}</small></span><b>→</b></Link>)}
 ...
 return <footer className={styles.footer} id="about">
  <div className={styles.footerInner}>
   ...
   <div className={styles.footerColumns}>
    ...
    <nav className={styles.footerNav} aria-label="Навигация в подвале">{columns.map(b=><div key={b.id} {...attrs(b)}><p>{b.values.title}</p>{b.items.filter(item=>item.href&&item.title&&item.title!=="Реквизиты"&&!/\/requisites\/?$/.test(item.href)).map((item,i)=><ContentLink key={i} href={item.href}>{item.title}</ContentLink>)}</div>)}</nav>
   </div>
