...
export function ProductPurchaseActions({product,quickBuy=true,compact=false,addLabel='В корзину'}:{product:Product;quickBuy?:boolean;compact?:boolean;addLabel?:string}){
 ...
 const disabled=closed||product.stock<=0;
 return <div className={`${styles.actions} ${compact?styles.compact:''}`} data-purchase-actions>
  ...
  {quantity>0&&product.stock>0?<div className={styles.stepper} role="group" aria-label={`Количество ${product.name} в корзине`}>
   <button type="button" aria-label={`Уменьшить количество ${product.name}`} onClick={()=>changeQuantity(product.id,quantity-1)}>−</button>
   <output aria-live="polite">{quantity}</output>
   <button type="button" aria-label={`Увеличить количество ${product.name}`} disabled={closed||quantity>=product.stock} onClick={()=>changeQuantity(product.id,quantity+1)}>+</button>
