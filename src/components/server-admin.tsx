...
function CatalogEditor({session,onExpired}:{session:StaffSession;onExpired:()=>void}){
 ...
 async function save(e:FormEvent){
  ...
  await run(async()=>{
   ...
   await api.save(product.id,cleaned,product.revision,session.csrfToken);await open(product.id);await list();reloadCatalog();setNotice(product.active?'Изменения сохранены и опубликованы.':'Черновик сохранён. Для появления на сайте опубликуйте товар.');
  });
 }
 ...
 return <><p>Новую карточку сохраните и опубликуйте. Изменения опубликованного товара применяются при сохранении. Остатки не влияют на публикацию.</p>
 ...
 <section className={styles.editor}>{!product?<p>Выберите товар или добавьте новый.</p>:<><header><h2>{draft.name||'Новый товар'}</h2><p>{lifecycleLabels[product.lifecycle??(product.active?'published':product.publishedAt?'unpublished':'draft')]} · {dirty?'Есть несохранённые правки':product.hasDraft?'Правки сохранены':'Черновик ещё не сохранён'}</p>
 <div className={styles.actions}>{!product.active&&<button disabled={busy||deleted||!product.hasDraft||dirty||issues.length>0} onClick={()=>setConfirm('publish')}>Опубликовать товар</button>}<button disabled={busy||dirty||!product.active} onClick={()=>setConfirm('unpublish')}>Снять с публикации</button>
 ...
 <form onSubmit={save} key={product.id+':'+product.revision}><fieldset disabled={busy||deleted}>
 <h3>Основная информация</h3><p>Пустые артикул и адрес карточки будут созданы автоматически. Артикул можно исправить только до первой публикации и использования в заказах или интеграциях. Архивирование не снимает это ограничение.</p><label>Артикул<input placeholder="Будет создан автоматически" maxLength={100} readOnly={product.active||!!product.publishedAt} value={draft.sku} onChange={e=>change({sku:e.target.value})}/></label>
