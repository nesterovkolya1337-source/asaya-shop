...
export function ImageCropEditor({source,value,type,device,ratio,onDevice,onChange}:Props){
 ...
 return <details className={styles.panel}><summary>Настроить кадр</summary><div className={styles.devices}><button type="button" aria-pressed={device==='desktop'} onClick={()=>onDevice('desktop')}>Компьютер</button><button type="button" aria-pressed={device==='mobile'} onClick={()=>onDevice('mobile')}>Телефон</button></div>
