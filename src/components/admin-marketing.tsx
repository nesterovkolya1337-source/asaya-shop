...
type Config={twoPercent:number;threePercent:number;freeShippingMinor:number;loyalty?:{cashbackPercent:number;maxRedemptionPercent:number}};
...
const equal=(a:Config,b:Config)=>fields.every(([key])=>a[key]===b[key])&&JSON.stringify(a.loyalty)===JSON.stringify(b.loyalty);
export function AdminMarketing({session,onExpired}:{session:StaffSession;onExpired:()=>void}){
 ...
 <button type="submit">Сохранить черновик</button><button type="button" disabled={dirty} onClick={()=>void act('publish')}>Опубликовать сохранённое</button><button type="button" onClick={()=>setEdit({...state.defaults})}>Вернуть стандартные значения</button>
