...
export function AdminSettings({session,onExpired,onOrder}:{session:StaffSession;onExpired:()=>void;onOrder:(id:string)=>void}){
 ...
 return <section aria-label="Настройки"><h1>Настройки</h1><nav className={styles.productTabs} aria-label="Разделы настроек">{[['store','Магазин'],['appearance','Оформление'],['integrations','Интеграции'],['system','Системное']].map(([id,label])=><button key={id} type="button" aria-pressed={tab===id} aria-controls={'settings-'+id} onClick={()=>setTab(id)}>{label}</button>)}</nav>
 ...
 {tab==='appearance'&&<div id="settings-appearance"><AdminAppearance csrf={session.csrfToken} onExpired={onExpired}/></div>}
