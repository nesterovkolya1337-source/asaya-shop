...
try{
 ...
 const mappingWarnings=await checkPublishedStockCoverage(db.pool,snapshot.items.map(i=>i.sku));
 if(mappingWarnings.length)console.log(JSON.stringify({stage:'mapping_warning',articles:mappingWarnings}));
 console.log('STOCK_PREFLIGHT_PASSED');
}catch(error){
 const known=['STOCK_PRODUCTION_PROFILE_INVALID','STOCK_PREFLIGHT_CONFIG_MISSING','STOCK_PREFLIGHT_NOT_PRODUCTION','STOCK_WAREHOUSE_SCOPE_MISMATCH','STOCK_SOURCE_CHANGE_REQUIRES_REVIEW','STOCK_SOURCE_STALE','STOCK_PREFLIGHT_UNKNOWN_SKUS','STOCK_SOURCE_UNAVAILABLE','STOCK_SOURCE_UNAUTHORIZED','STOCK_SOURCE_RATE_LIMITED'];
