...
export class OrderTracking{
 ...
 async webhook(raw:unknown){
  ...
  await this.db.transaction(async tx=>{
   ...
   const eventId=hash(canonical([e.uuid,e.attributes.code,e.attributes.status_date_time]));
 ...
 async refresh(orderId:string){
  ...
  await this.db.transaction(async tx=>{
   ...
   const previous=(await tx.query('SELECT * FROM order_logistics WHERE order_id=$1 FOR UPDATE',[orderId])).rows[0];
   ...
   await tx.query(`UPDATE order_logistics SET cdek_uuid=$2,delivery_status=$3,delivery_raw_status=$4,delivery_occurred_at=$5,delivery_synced_at=$6,
    delivery_requested_at=CASE WHEN delivery_requested_at<=$6 THEN NULL ELSE delivery_requested_at END,delivery_next_attempt_at=NULL,delivery_attempts=0 WHERE order_id=$1`,[orderId,response.uuid,current.status,current.rawStatus,current.occurredAt,at]);
   const legacy:Partial<Record<DeliveryStatus,string>>={handed_to_cdek:'shipped',in_transit:'shipped',out_for_delivery:'shipped',ready_for_pickup:'arrived_to_pickup_point',delivered:'delivered',returning:'returned'};
 ...
 async requestStale(orderId:string,userId:string){
  await this.db.pool.query(`UPDATE order_logistics l SET delivery_requested_at=$3 FROM orders o WHERE o.id=l.order_id AND o.id=$1 AND o.user_id=$2
   AND l.account_id=$4 AND l.environment=$5 AND l.tracking_number IS NOT NULL AND l.delivery_status IS DISTINCT FROM 'delivered'
 ...
 async due(limit=20){
  ...
  return (await this.db.pool.query(`SELECT order_id FROM order_logistics WHERE account_id=$1 AND environment=$2 AND tracking_number IS NOT NULL
   ...
   AND (delivery_requested_at IS NOT NULL OR (COALESCE(delivery_status,'created') NOT IN ('delivered','cancelled','returning') AND (delivery_synced_at IS NULL OR delivery_synced_at<$3::timestamptz-interval '24 hours')))
 ...
 async failed(orderId:string){await this.db.pool.query(`UPDATE order_logistics SET delivery_attempts=LEAST(delivery_attempts+1,20),
  delivery_next_attempt_at=$2::timestamptz+make_interval(secs => LEAST(21600,300*power(2,LEAST(delivery_attempts,7)))::double precision)
...
export async function trackingProjection(db:Database,order:{id:string;status:string;payment_status:string}){
 const row=(await db.pool.query('SELECT * FROM order_logistics WHERE order_id=$1',[order.id])).rows[0];if(!row)return undefined;
