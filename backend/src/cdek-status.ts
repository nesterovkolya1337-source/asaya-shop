...
export type DeliveryStatus='created'|'handed_to_cdek'|'in_transit'|'out_for_delivery'|'ready_for_pickup'|'delivered'|'cancelled'|'returning'|'review';
...
export function customerOrderStatus(input:{payment:string;order:string;fulfillment?:FulfillmentStatus|null;delivery?:DeliveryStatus|null}):CustomerOrderStatus{
 ...
 if(input.delivery==='review'||input.fulfillment==='review')return 'review';
 ...
 if(input.fulfillment==='handed_to_delivery')return 'handed_to_cdek';
 if(input.fulfillment&&['accepted','assembling','assembled'].includes(input.fulfillment))return 'assembling';
 return 'paid';
}
