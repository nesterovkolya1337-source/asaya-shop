...
export const stockSettingsSchema=z.object({
 ...
 pollSeconds:z.number().int().min(30).max(300).default(60),
 maxAgeSeconds:z.number().int().min(60).max(3600).default(1800)
...
export class CdekStockFeed {
 ...
 async read():Promise<StockSnapshot>{
  ...
  }catch{throw new DomainError('STOCK_SOURCE_UNAVAILABLE',503);}
 }
}
