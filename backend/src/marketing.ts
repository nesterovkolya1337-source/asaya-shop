...
export const marketingSchema=z.object({loyalty:z.object({cashbackPercent:z.number().int().min(0).max(100),maxRedemptionPercent:z.number().int().min(0).max(100)}).strict().optional(),twoPercent:z.number().int().min(0).max(100),threePercent:z.number().int().min(0).max(100),freeShippingMinor:z.number().int().min(0).max(100000000)}).strict();
...
export class Marketing {
 ...
 async change(actor:string,action:'save'|'publish'|'restore'|'defaults',raw:unknown){
  ...
  return this.db.transaction(async tx=>{
   ...
   const settings=action==='publish'?row.draft:action==='restore'?row.defaults:{...body.settings,loyalty:body.settings?.loyalty??row[field].loyalty??{cashbackPercent:3,maxRedemptionPercent:20}};
