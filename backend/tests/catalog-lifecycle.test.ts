...
test('Published Rich Content saves immediately; hidden and draft preserve visibility and canonical prices',async()=>{
 const actor=randomUUID(),id=randomUUID(),catalog=new AdminCatalog(ctx.db);
 await ctx.db.pool.query("INSERT INTO users(id,role) VALUES($1,'admin')",[actor]);
 ...
 assert.notDeepEqual((await ctx.db.pool.query('SELECT published FROM product_editor WHERE product_id=$1',[id])).rows[0].published,liveBefore);
 ...
 await catalog.save(actor,id,{...draft,revision:4,regularMinor:1,finalMinor:1,content:{...draft.content,pdp:{...pdp,enabled:false}}});await catalog.publish(actor,id,{revision:5});
