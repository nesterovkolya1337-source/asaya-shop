...
test('apply preserves visibility and draft lifecycle, validates atomically and rejects stale versions',async()=>{
 const {id,d,api}=await seed('APPLY');await api.save(actor,id,{...d,revision:1,name:'Draft'},true);assert.equal((await api.detail(id)).active,false);
 await api.publish(actor,id,{revision:2});await api.save(actor,id,{...d,revision:3,name:'Visible',finalMinor:8000},true);
 const commerce=new CommerceService(ctx.db);assert.equal((await commerce.catalog()).find(p=>p.sku==='APPLY')!.finalMinor,8000);assert.equal((await api.detail(id)).hasUnpublishedChanges,false);
 await assert.rejects(api.save(actor,id,{...d,revision:3},true),/EDIT_CONFLICT/);
 await assert.rejects(api.save(actor,id,{...d,revision:4,content:{...d.content,ingredients:''}},true),/PUBLISH_INCOMPLETE/);assert.equal((await api.detail(id)).revision,4);
 await api.unpublish(actor,id,{revision:4});await api.save(actor,id,{...d,revision:5,name:'Hidden'},true);assert.equal((await api.detail(id)).lifecycle,'unpublished');assert.equal((await commerce.catalog()).some(p=>p.sku==='APPLY'),false);
 await api.publish(actor,id,{revision:6});assert.equal((await commerce.catalog()).find(p=>p.sku==='APPLY')!.name,'Hidden');
});
